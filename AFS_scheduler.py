...
def top_priority(q, gpu_type, worker_nums):
    j_opt = random.choice(q)
    for j in q:
        j_a, j_b = j_opt, j

        if worker_nums[j_a] == 0 and worker_nums[j_b] == 0:
            if (available_jobs[j_a]['iteration'] - available_jobs[j_a]['progress']) * cal_iteration_time(gpu_type, available_jobs[j_a]['job_type'], available_jobs[j_a]['batch_size'], 1) < \
                    (available_jobs[j_b]['iteration'] - available_jobs[j_b]['progress']) * cal_iteration_time(gpu_type, available_jobs[j_b]['job_type'], available_jobs[j_b]['batch_size'], 1):
                j_opt = j_a
            else:
                j_opt = j_b
        else:
            len_a = 9999999999999999999 if worker_nums[j_a] == 0 else (available_jobs[j_a]['iteration'] - available_jobs[j_a]['progress']) * cal_iteration_time(gpu_type, available_jobs[j_a]['job_type'], available_jobs[j_a]['batch_size'], worker_nums[j_a])
            len_b = 9999999999999999999 if worker_nums[j_b] == 0 else (available_jobs[j_b]['iteration'] - available_jobs[j_b]['progress']) * cal_iteration_time(gpu_type, available_jobs[j_b]['job_type'], available_jobs[j_b]['batch_size'], worker_nums[j_b])
            if len_a >= len_b:
                j_a, j_b = j_b, j_a

            p_b_prime = cal_throughput(gpu_type, available_jobs[j_b]['job_type'], available_jobs[j_b]['batch_size'], worker_nums[j_b] + 1)
            p_b = cal_throughput(gpu_type, available_jobs[j_b]['job_type'], available_jobs[j_b]['batch_size'], worker_nums[j_b]) if worker_nums[j_b] > 0 else 0
            p_a_prime = cal_throughput(gpu_type, available_jobs[j_a]['job_type'], available_jobs[j_a]['batch_size'], worker_nums[j_a] + 1)
            p_a = cal_throughput(gpu_type, available_jobs[j_a]['job_type'], available_jobs[j_a]['batch_size'], worker_nums[j_a]) if worker_nums[j_a] > 0 else 0

            if ((p_b_prime - p_b) / p_b_prime) > ((p_a_prime - p_a) / p_a):
                j_opt = j_b
            else:
                j_opt = j_a
    return j_opt
...
def schedule():
    result = {}
    # iterate over queues for each gpu type
    for i in range(len(queues)):
        gpu_type = gpu_types[i]
        available_workers = gpu_nums[gpu_type]

        # record the worker of each job
        worker_nums = {}
        for j in queues[i]:
            worker_nums[j] = 0

        # jobs that has worker equals to its limit
        full_jobs = []
        for _ in range(available_workers):
            # if all jobs in this queue are full, exit earlier
            if len(full_jobs) == len(queues[i]):
                break
            j = top_priority([item for item in queues[i] if item not in full_jobs], gpu_type, worker_nums)
            worker_nums[j] += 1
            # cannot beyond per job limit
            if worker_nums[j] == all_job_limit:
                full_jobs.append(j)

        for j in queues[i]:
            if worker_nums[j] > 0:
                result[j] = {'type': gpu_type, 'job_type': available_jobs[j]['job_type'], 'bs': available_jobs[j]['batch_size'], 'num': worker_nums[j]}
    return result
