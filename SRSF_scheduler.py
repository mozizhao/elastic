...
def schedule():
    ...
    for i in range(len(queues)):
        ...
        for _ in range(int(gpu_nums[gpu_type] // all_job_limit)):
            if len([j for j in queues[i] if j not in result.keys()]) <= 0:
                break
            j = min([j for j in queues[i] if j not in result.keys()], key=(lambda j: (available_jobs[j]['iteration'] - available_jobs[j]['progress']) * cal_iteration_time(gpu_type, available_jobs[j]['job_type'], available_jobs[j]['batch_size'], all_job_limit)))
            result[j] = {'type': gpu_type, 'num': all_job_limit, }

        # # jobs sorted by their processing time in descending order.
        # job_ids = sorted(copy.deepcopy(available_jobs), key=(lambda jid: ((available_jobs[jid]['iteration'] - available_jobs[jid]['progress']) * cal_iteration_time(gpu_type, available_jobs[jid]['job_type'], all_job_limit)) / all_job_limit), reverse=False)
        #
        # for jid in job_ids:
        #     if jid not in result and sum(list(result[k]['num'] for k in result if result[k]['type'] == gpu_type)) < gpu_nums[gpu_type]:
        #         result[jid] = {'type': gpu_type, 'num': min(gpu_nums[gpu_type] - sum(list(result[k]['num'] for k in result if result[k]['type'] == gpu_type)), available_jobs[jid]['limit'])}
        #
        # assert gpu_nums[gpu_type] - sum(list(result[k]['num'] for k in result if result[k]['type'] == gpu_type)) >= 0
